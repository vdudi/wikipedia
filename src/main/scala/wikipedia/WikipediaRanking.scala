...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    ...
    //rdd.aggregate(0)((i,wiki)=>
    //  wiki.text.split(" ").filter(lang eq _).map(_ => 1).reduceLeft(_ + _), _ + _)

    //val aggregateAcc : (Int, String, WikipediaArticle) => Int = (acc, lang, wpa) => if(wpa.mentionsLanguage(lang)) acc+1 else acc
    //rdd.aggregate[Int](0) (count, lang, )
    rdd.aggregate[Int](0)((count, article) => if(article.mentionsLanguage(lang)) count+1 else count,
                    (x, y) => (x + y))
